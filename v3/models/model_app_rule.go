...
// AppRule These rules govern what flows are allowed for app rules. Target group is a required attribute. Empty inbound_allow_list will not allow anything into the target group. Empty outbound_allow_list will not allow anything from the target group. 
type AppRule struct {
...
func (o *AppRule) UnmarshalJSON(data []byte) (err error) {
	varAppRule := _AppRule{}

	err = json.Unmarshal(data, &varAppRule)

	if err != nil {
		return err
	}

	*o = AppRule(varAppRule)
