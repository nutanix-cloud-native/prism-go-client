...
// ADRule These rules govern what flows are allowed for AD categories. Target group is a required attribute. Empty inbound_allow_list will not allow anything into the target group. Empty outbound_allow_list will not allow anything from the target group. 
type ADRule struct {
...
func (o *ADRule) UnmarshalJSON(data []byte) (err error) {
	varADRule := _ADRule{}

	err = json.Unmarshal(data, &varADRule)
