...
// UserStatus The status of a REST API call. Only used when there is a failure to report. 
...
func (o *UserStatus) UnmarshalJSON(data []byte) (err error) {
	varUserStatus := _UserStatus{}

	err = json.Unmarshal(data, &varUserStatus)

	if err != nil {
		return err
	}

	*o = UserStatus(varUserStatus)
