...
func TestValidateCredentials(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			key:     "key3",
			wantErr: "API credentials(either a username and password, or an API key) is required for authentication in cachedClientParams with key key3",
		},
		{
			...
			key:     "key4",
			wantErr: "API credentials(either a username and password, or an API key) is required for authentication in cachedClientParams with key key4",
		},
	}
