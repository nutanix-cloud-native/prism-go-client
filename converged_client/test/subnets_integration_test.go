...
func TestSubnetsService_Get(t *testing.T) {
	...
	subnets, err := client.Subnets.List(ctx, models.WithPage(0))
	assert.Nil(t, err)
	assert.True(t, len(subnets) > 0)

	_, err = client.Subnets.Get(ctx, *subnets[0].ExtId)
	assert.Nil(t, err)
}
...
func TestSubnetsService_List(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			_, err := client.Subnets.List(ctx, tt.opts...)
			assert.Nil(t, err)
		})
	}
}
...
func TestSubnetsService_NewIterator(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			opts: []models.ODataOption{
				...
				models.WithExpand("virtualSwitch"),
			},
		},
	}
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			iterator := client.Subnets.NewIterator(ctx, tt.opts...)
