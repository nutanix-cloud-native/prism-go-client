...
generate-v3-models: ## Generate V3 models using go-swagger
	swagger generate model \
		...
		--model=vm \
		--model=vm_resources \
		--model=vm_vtpm_config \
		--model=vm_power_state_mechanism \
		--model=vm_guest_power_state_transition_config \
		--model=serial_port \
		--model=vm_apc_config \
		--model=vm_boot_config \
		--model=vm_boot_device \
		--model=vm_disk \
		--model=disk_storage_config \
		--model=disk_address \
		--model=vm_gpu \
		--model=guest_customization \
		--model=custom_generic_key_value_pair \
		--model=guest_tools_spec \
		--model=vm_nic \
		--model=ip_address \
		--model=subnet_reference \
		--model=nutanix_guest_tools_spec \
		--model=vm_storage_config \
		--model=flash_mode \
		--model=storage_qos_policy_config_input \
		--model=vm_vnuma_config \
		--model=vm_def_status \
		--model=vm_resources_def_status \
		--model=vm_disk_output_status \
		--model=effective_storage_config \
		--model=vm_gpu_output_status \
		--model=guest_customization_status \
		--model=nutanix_guest_tools_status \
		--model=guest_tools_status \
		--model=vm_nic_output_status \
		--model=protection_policy_state \
		--model=policy_compliance_state \
		--model=protection_info \
		--model=recovery_plan_policy_state \
		--model=recovery_plan_info \
		--model=vm_storage_config_status \
		--model=storage_qos_policy_config_output \
		--model=vm_vtpm_status \
		--model=vm_intent_resource \
		--model=vm_metadata
