...
import (
	...
	"github.com/nutanix-cloud-native/prism-go-client/converged/mocks"
...
func TestCustomErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:      converged.ErrTaskNotComplete,
			expected: "task not yet complete",
		},
		{
			name:     "ErrTaskResultsNotReady",
			err:      converged.ErrTaskResultsNotReady,
			expected: "task results not ready",
		},
		{
			name:     "ErrTaskFailed",
			err:      converged.ErrTaskFailed,
...
func TestTaskStatusConstants(t *testing.T) {
	tests := []struct {
		name     string
		status   converged.TaskStatus
		expected string
	}{
		{
			name:     "TaskStatusFailed",
			status:   converged.TaskStatusFailed,
			expected: "FAILED",
		},
		{
			name:     "TaskStatusQueued",
			status:   converged.TaskStatusQueued,
			expected: "QUEUED",
		},
		{
			name:     "TaskStatusRunning",
			status:   converged.TaskStatusRunning,
			expected: "RUNNING",
		},
		{
			name:     "TaskStatusSuspended",
			status:   converged.TaskStatusSuspended,
			expected: "SUSPENDED",
		},
		{
			name:     "TaskStatusSucceeded",
			status:   converged.TaskStatusSucceeded,
			expected: "SUCCEEDED",
		},
		{
			name:     "TaskStatusCanceled",
			status:   converged.TaskStatusCanceled,
			expected: "CANCELED",
		},
		{
			name:     "TaskStatusCanceling",
			status:   converged.TaskStatusCanceling,
			expected: "CANCELLING",
		},
		{
			name:     "TaskStatusUnknown",
			status:   converged.TaskStatusUnknown,
			expected: "UNKNOWN",
		},
		{
			name:     "TaskStatusRedacted",
			status:   converged.TaskStatusRedacted,
			expected: "REDACTED",
		},
	}
...
func TestTaskStatusComparison(t *testing.T) {
	...
	assert.NotEqual(t, converged.TaskStatusQueued, converged.TaskStatusRunning)
	assert.NotEqual(t, converged.TaskStatusRunning, converged.TaskStatusSucceeded)
	assert.NotEqual(t, converged.TaskStatusSucceeded, converged.TaskStatusFailed)

	// Test that the same constants are equal
	assert.Equal(t, converged.TaskStatusQueued, converged.TaskStatusQueued)
	assert.Equal(t, converged.TaskStatusSucceeded, converged.TaskStatusSucceeded)
...
func TestErrorIdentity(t *testing.T) {
	...
	assert.NotEqual(t, converged.ErrTaskNotComplete, converged.ErrTaskFailed)
	assert.NotEqual(t, converged.ErrTaskFailed, converged.ErrTaskResultsNotReady)

	// Test that the same error variables are identical
	assert.Equal(t, converged.ErrTaskNotComplete, converged.ErrTaskNotComplete)
	assert.Equal(t, converged.ErrTaskFailed, converged.ErrTaskFailed)

	// Test error wrapping behavior
	wrappedErr := errors.Join(converged.ErrTaskFailed, errors.New("additional context"))
	assert.ErrorIs(t, wrappedErr, converged.ErrTaskFailed)
}
...
func TestWithPage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithPage(tt.page)
...
func TestWithLimit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithLimit(tt.limit)
			err := option(mockOData)
...
func TestWithFilter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithFilter(tt.filter)
			err := option(mockOData)
...
func TestWithOrderBy(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithOrderBy(tt.orderBy)
			err := option(mockOData)
...
func TestWithExpand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithExpand(tt.expand)
			err := option(mockOData)
...
func TestWithSelect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithSelect(tt.selectFields)
			err := option(mockOData)
...
func TestWithApply(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			option := converged.WithApply(tt.apply)
			err := option(mockOData)
...
func TestOperationInterface(t *testing.T) {
	...
	// Test Status
	mockOperation.EXPECT().Status().Return(converged.TaskStatusSucceeded)
	assert.Equal(t, converged.TaskStatusSucceeded, mockOperation.Status())
...
func TestNoEntityGetter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := converged.NoEntityGetter(tt.uuid)
