...
func TestVMsInterface(t *testing.T) {
	...
	mockOperationDelete := mocks.NewMockOperation[converged.NoEntity](ctrl)
...
func TestVMsInterfaceComposition(t *testing.T) {
	...
	var getter converged.Getter[TestVM] = mockVMs
	var lister converged.Lister[TestVM] = mockVMs
	var creator converged.Creator[TestVM] = mockVMs
	var updater converged.Updater[TestVM] = mockVMs
	var deleter converged.Deleter[TestVM] = mockVMs
	var asyncCreator converged.AsyncCreator[TestVM] = mockVMs
	var asyncUpdater converged.AsyncUpdater[TestVM] = mockVMs
	var asyncDeleter converged.AsyncDeleter[TestVM] = mockVMs
...
func TestVMsWithODataOptions(t *testing.T) {
	...
	mockVMs.EXPECT().List(ctx, gomock.Any()).Return(expectedVMs, nil)
	entities, err := mockVMs.List(ctx, converged.WithPage(1), converged.WithLimit(10))
	assert.NoError(t, err)
	...
	resultIterator := mockVMs.NewIterator(ctx, converged.WithOrderBy("name"))
	count := 0
