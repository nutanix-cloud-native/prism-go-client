...
// across calls. Calling WithExtraHeaders with a nil or empty map is a no-op.
func WithExtraHeaders(h http.Header) UploadOption {
	return func(opts *UploadOptions) {
		...
		if opts.ExtraHeaders == nil {
			opts.ExtraHeaders = make(http.Header, len(h))
		}
		for k, vs := range h {
			for _, v := range vs {
				opts.ExtraHeaders.Add(k, v)
			}
		}
	}
}
...
type Images[Image, FileDetail any] interface {
	...
	// configure the upload — see WithUploadProjectUUID and WithExtraHeaders.
	// Calling Upload with no options preserves the pre-options behaviour exactly.
